...
export default class App extends React.Component {
  ...
  getIdeas() {
    ...
    let airtableUrl = "https://api.airtable.com/v0/app6fawiMn3BOBlpR/cities?&view=Grid%20view";

    // Needed for Airtable authorization, replace with your own API key
    let requestOptions = {
      headers: new Headers({
        'Authorization': 'Bearer key6eMO6QFxyRnBQv'
      })
    };
  ...
  upvoteIdea(data, secId, rowId, rowMap) {
    ...
    let airtableUrl = "https://api.airtable.com/v0/app6fawiMn3BOBlpR/cities/" + data.id;

    // Needed for Airtable authorization
    let requestOptions = {
      method: 'PATCH',
      headers: new Headers({
        'Authorization': 'Bearer key6eMO6QFxyRnBQv', // replace with your own API key
  ...
  render() {
    ...
    return (
      <Container>
        <Header>
          <Body>
            <Title>America's Largest Cities: Best Restaurant Scene</Title>
          </Body>
        </Header>
        <Content>
          <List
